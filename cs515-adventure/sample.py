...
class Sample:
    ...
    def help():
        ...
        print("  help")
def main(file):
    data = Sample.load_rooms(file)
    current_room = 0
    inventory = []
    Sample.print_room_info(data[current_room])
    ...
    while True:
        ...
        if action == 'go':
            ...
            current_room = Sample.move_to_next_room(data, current_room, direction)
            ...
            Sample.print_room_info(data[current_room])
                ...
                Sample.get_item(data, current_room, item, inventory)

        elif action == 'inventory':
            Sample.show_inventory(inventory)

        elif action == 'drop':
            print("Specify an item to drop.")
        elif action.startswith('drop '):
            item = action.split(' ', 1)[-1]
            Sample.drop_item(data, current_room, item, inventory)
